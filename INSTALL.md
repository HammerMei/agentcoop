...
> says `~/.agentcoop`. The installer ends by printing the `export COOP_HOME=…` line to
> add to your shell startup file; add it yourself (a manual install too). `COOP_HOME` is for that conflict case only:
