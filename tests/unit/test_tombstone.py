...
class TestTombstone(unittest.TestCase):
    def test_explains_the_rename_and_both_ways_out_then_exits_1(self):
        ...
        self.assertIn("git checkout v0.5.2 && uv sync", text)             # stay on v0.5
        self.assertIn("~/.agent-chat-gateway/repo", text)                 # where that install lives
    ...
    def test_forwards_nothing(self):
        ...
        public = [n for n in dir(tombstone) if not n.startswith("_") and n not in ("sys",)]
        self.assertEqual(sorted(public), ["MESSAGE", "main"])
