...
into `builtin/`; `coop upgrade` overwrites every shipped file there
unconditionally and touches nothing else, so files the CLIs create at run
time (`.claude/settings.local.json`, session state) survive. The `builtin/`
...
A bot's persona is the `CLAUDE.md` in its working directory. OpenCode falls
back to `CLAUDE.md` when no `AGENTS.md` is present, so one file serves both
backends there too.

### 3.2 Bootstrap

At the start of every session the keeper establishes the machine's state
with four commands: whether `~/.agentcoop/admin-profiles.yaml` exists,
`coop config show --json` (which also reports whether the running daemon
differs from the file), `coop status`, and `coop config backends --json`.
There is no aggregate "doctor" command; one is added only if these four
prove insufficient in use.

When `admin-profiles.yaml` is absent the keeper asks which platform, the
...
Bootstrap also creates, if absent, the `tool_presets` the agent template
refers to and one `default` entry in each of `connector_templates`,
`agent_templates` and `watcher_templates`. These hold the settings the
...
`admin-profiles.yaml`. Bot passwords are generated by the keeper (`openssl
rand`) into a `0600` temporary file and passed to both CLIs by path —
...
keeper reads configuration only through `coop config show --json`, whose
output is secret-redacted. The deny rules do not govern the shell: a `cat` of
...
write. The first bot in an empty
`config.yaml` is the one case this ordering does not cover — the file is
invalid until both a connector and an agent exist (`gateway/config.py`
requires at least one of each) — so the first bot is written as one
`coop config patch --file` carrying all three entries.

A name that already exists is refused by `coop config add`; the keeper
...
A room change is a `coop config patch` on the rule, followed by the apply
step (§3.7). A persona change rewrites the agent's `CLAUDE.md` and changes
...
`coop reset '<agent>@<profile>:*'` — a fresh session, which does read the
file — as a confirmed runtime step, and states that on Claude Code it is
optional.
...
Removing a bot is confirmed once and then done in full: the rule and
connector are removed (the agent too, when this was its last bot); the daemon
is reloaded, whose reconciliation expires the records no rule covers; the
server account is deleted with `coop-provision delete-user`; the agent's
directory under `agents/user/` is deleted, but a directory anywhere else is
left untouched; and the account is removed from the agent-chain list (§3.6).
...
accepted for this version and documented. A hand-written connector that does
not inherit `default` is patched individually, and the plan says so.
...
shell commands the skills run — `mkdir` for an agent's directory, `openssl
rand` and `rm` for the password file, `ls` and `which` for the session-start
checks — and the `Read`/`Write`/`Edit` tools under `~/.agentcoop/agents/`;
...
`coop-provision` gains three things: `--password-file` on `create-user`;
...
exists; and `<profile> check`, which runs the existing `connect()` — an
authenticated `get_me` plus team resolution — and exits non-zero on
failure. The default location
...
1. **Command surface** — §3.9 in full: `coop config add/remove/patch/backends`,
   `coop-provision init`/`check` and `--password-file`, the
   `admin-profiles.yaml` default path. Independently testable and useful
   without the keeper.
...
8. Refusals: a duplicate name, a backend that is not installed, a credential
   pasted into the conversation, and — after the second bot on a server —
