...
closed by the owner: the default stays `false`; per-command guest restrictions are not the
security model (ADR 0001 — anything an agent knows is reachable by whoever can talk to it).
